...
var PDFShow = function (fileid) {
    document.getElementById("renderMsg").style.display = "none";
    var myPDF = new PDFObject({ url: decode64Han(fileid) }).embed();
...
var PDFObject = function (obj) {
    ...
    if (!obj.pdfOpenParms) {
        obj.pdfOpenParams = {
            navpanes: 1, statusbar: 1, toolbar: 1, view: "FitV", pagemode: "thumbs"
        };
    }
    ...
    var hasReaderActiveX = function () {
        ...
        if (window.ActiveXObject) {
            ...
            try {
                ...
                if (!axObj) {
                    axObj = new ActiveXObject("AcroPDF.PDF.1");
                }
                ...
                if (!axObj) {
                    for (x = 1; x < 15; x++) {
                        try {
                            axObj = eval("new ActiveXObject('PDF.PdfCtrl." + x + "');");
                ...
                if (!axObj) {
                    axObj = new ActiveXObject('AcroExch.Document');
                }

                if (!axObj) {
                    axObj = new ActiveXObject("theAdobePdfCOMObject");
                }
    ...
    var pluginFound = function () {
        ...
        } else if (checkBrowser(a.toLowerCase(), "rv:") || checkBrowser(a, "Macintosh") || checkBrowser(a.toLowerCase(), "opera") || checkBrowser(a.toLowerCase(), "firefox") || checkBrowser(a.toLowerCase(), "chrome") || checkBrowser(a.toLowerCase(), "safari")) {
            type = "generic";
    ...
    var embed = function (targetID) {
        if (!pluginTypeFound) {
            ...
            //document.location.href = "http://get.adobe.com/reader";

            setTimeout(showErrorMsg, 600)
        ...
        if (pluginTypeFound == "mobile") {
            //document.location.href = "http://hrpt.seoyoneh.com/viewerJS/#." + url.substring(0, url.indexOf("#"));
            document.location.href = url;
        }
        else {
            targetNode.innerHTML = '<object id="embedPDF" data="' + url + '" type="application/pdf" width="' + width + '" height="' + height + '"></object>';
        }
