...
var giTimerCnt = 0;
function init() {
    if (goAgent.isWin && !goAgent.isFF) {

        try {
            var sVer;
            if (typeof (RexCtl) != "undefined") sVer = RexCtl.GetVersion();
...
function OnLoad() {
    ...
    var sParam = window.location.search.split("&")[0];
...
function fnOpen(oOOF) {
    ...
    try {
        ...
        RexCtl.SetCSS("appearance.toolbar.button.find.visible=1");
...
function OnBeforePrint(printname, frompage, topage, copies, cancel) {
    if (goOOF.event.beforeprint != null) {
        goOOF.event.beforeprint(RexCtl, "beforeprint", { "printname": printname, "frompage": frompage, "topage": topage, "copies": copies, "cancel": cancel });
    }
}
...
function fnTimerExportUpload() {
    RexCtl.ExportUpload(goOOF.saveupload.dialog, goOOF.saveupload.filetype, goOOF.saveupload.fileName,
			        goOOF.saveupload.startpage, goOOF.saveupload.endpage, goOOF.saveupload.Option, goOOF.saveupload.uploadurl);
}
