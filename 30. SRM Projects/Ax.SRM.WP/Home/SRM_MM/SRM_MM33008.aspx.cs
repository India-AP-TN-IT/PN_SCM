...
namespace Ax.SRM.WP.Home.SRM_MM
{
    ...
    public partial class SRM_MM33008 : BasePage
    {
        private string pakageName = "APG_SRM_MM33008_AP";
        ...
        private DataSet getDataSet()
        {
            ...
            return EPClientHelper.ExecuteDataSet(string.Format("{0}.{1}", pakageName, "INQUERY_HEADER"), param);
        }
        ...
        /// </summary>
        /// <returns></returns>
        private DataSet getDataSetDetail(string rcvYear, string rcvNo, string rcvType)
        {
            HEParameterSet param = new HEParameterSet();

            param.Add("CORCD", Util.UserInfo.CorporationCode);
            param.Add("BIZCD", this.cbo01_BIZCD.Value);
            param.Add("ACC_RCV_NO", rcvNo);
            param.Add("ACC_RCV_YEAR", rcvYear);
            param.Add("VINCD", "A3" + txt01_VINCD.Value);
            param.Add("PARTNO", txt01_FPARTNO.Value);
            param.Add("LANG_SET", this.UserInfo.LanguageShort);            
            
            string procedure = "INQUERY_" + rcvType;
        ...
        private void Excel_Export(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                {
                    string rcvYear = ACC_RCV_YEAR.Value.ToString();
                    string rcvNo = ACC_RCV_NO.Value.ToString();
                    string rcvType = ACC_RCV_NO.Value.ToString();

                    result = getDataSetDetail(rcvYear, rcvNo, rcvType);
                }
                else
                {
                    result = getDataSet();
        ...
        [DirectMethod]
        public void Grid01_Cell_Click(string rcvYear, string rcvNo, string rcvType)
        {
            DataSet resultDetail = getDataSetDetail(rcvYear, rcvNo, rcvType);
