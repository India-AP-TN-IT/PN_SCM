...
namespace Ax.SRM.WP.Home.SRM_ALC
{
    ...
    public partial class SRM_ALC30003 : BasePage
    {
        ...
        {
            this.cbo01_PLANT_GB.SetValue("7");
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (cbo01_DIVISION.Text.ToString().Equals("ON-LINE")) //온라인일 경우 출력하기 위한 차수를 넣어놓는다.
                {
                    ...
                    if (ds02.Tables[0].Rows.Count > 0)
                    {
                        ...
                        this.txt01_MAX_SEQ.Text = LASTLSEQ;
        ...
        public void Reset()
        {
            ...
            this.txt01_MAX_SEQ.SetValue(string.Empty);
        ...
        private DataSet InitDate()
        {
            ...
            return EPClientHelper.ExecuteDataSet(string.Format("{0}.{1}", pakageName, "INQUERY_INIT_DATE"), set);
        }
        ...
        private DataSet Lseq()
        {
            ...
            return EPClientHelper.ExecuteDataSet(string.Format("{0}.{1}", pakageName, "INQUERY_MAX_LSEQ"), set);
        }
        ...
        // 자동 출력
        private void AutoPrint(DataSet ds02)
        {
            int GridCnt = ds02.Tables[0].Rows.Count;
            int intChasu_Qty = this.cbo01_PLANT_GB.Value.ToString() != "7" ? Convert.ToInt32(this.txt01_CHASU_QTY.Text) : Convert.ToInt32(this.txt01_CHASU_QTY.Text) * _DASICNT;

            if (this.cbo01_DIVISION.Value.Equals("ON-LINE") && GridCnt == intChasu_Qty)
            {
                HEParameterSet set = new HEParameterSet();
                set.Add("CORCD", Util.UserInfo.CorporationCode);
                set.Add("BIZCD", Util.UserInfo.BusinessCode);
                ...
                set.Add("PLANT", this.cbo01_PLANT_GB.Value);
                set.Add("LINE", this.cbo01_LINE_GB.Value);
                set.Add("CHASU", this.txt02_CHASU.Text);
                set.Add("USER_ID", Util.UserInfo.UserID);
                set.Add("LANG_SET", Util.UserInfo.LanguageShort);

                DataSet ds = EPClientHelper.ExecuteDataSet(string.Format("{0}.{1}", pakageName, "INQUERY_PRINT"), set);

                if (ds.Tables[0].Rows[0]["CNT"].ToString().Equals("0"))
                {
                    EPClientHelper.ExecuteNonQuery(string.Format("{0}.{1}", pakageName, "INSERT_PRINT"), set);
        ...
        private void Print()
        {
            try
            {
                ...
                {
                    report.ReportName = "1000/SRM_ALC/SRM_ALC30003_N7PLANT";  // /Report 폴더의하의 경로 ( 확장자 .reb 는 제외 ) / 리포트 파일은 법인코드 경로 아래에 모아두도록 함
                ...
                {
                    report.ReportName = "1000/SRM_ALC/SRM_ALC30003_7PLANT";  // /Report 폴더의하의 경로 ( 확장자 .reb 는 제외 ) / 리포트 파일은 법인코드 경로 아래에 모아두도록 함
        ...
        {
            if (this.cdx01_VENDCD.IsEmpty || this.cbo01_DIVISION.IsEmpty || this.df01_QUT_DATE.IsEmpty)
                return;
