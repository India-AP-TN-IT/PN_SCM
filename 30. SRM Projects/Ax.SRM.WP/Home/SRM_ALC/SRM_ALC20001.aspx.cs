...
{
    public partial class SRM_ALC20001 : BasePage
    {
        private string pakageName = "APG_SRM_ALC20001";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!X.IsAjaxRequest)
                {
                    ...
                    dt.Rows.Add("ALCDT", "ALCDT");
                    dt.Rows.Add("ALCSE", "ALCSE");
                    dt.Rows.Add("HDL", "HDL");
                    dt.Rows.Add("PKG", "PKG");
                    Library.ComboDataBind(this.cbo02_FILED_NM1, dt, true, false);
                    Library.ComboDataBind(this.cbo02_FILED_NM2, dt, true, false);
        ...
        public void Save(object sender, DirectEventArgs e)
        {
            ...
            try
            {
                ...
                (
                   "CORCD", "BIZCD", "VEND_CD", "PLANT_GB", "LINE_GB", "CHASU_QTY", "FIELD_NM1", "FIELD_NM2", "USER_ID", "LANG_SET"
                );
                ...
                for (int i = 0; i < parameters.Length; i++)
                {
                    ...
                    (
                        Util.UserInfo.CorporationCode, Util.UserInfo.BusinessCode, this.cdx01_VENDCD.Value, parameters[i]["PLANT_GB"], parameters[i]["LINE_GB"]
                        , parameters[i]["CHASU_QTY"], parameters[i]["FIELD_NM1"], parameters[i]["FIELD_NM2"]
        ...
        public void Delete(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                DataSet param = Util.GetDataSourceSchema
                (
                   "CORCD", "BIZCD", "PLANT_GB", "LINE_GB", "VEND_CD", "USER_ID", "LANG_SET"
                );
                ...
                for (int i = 0; i < parameters.Length; i++)
                {
                    if (bool.Parse(parameters[i]["CHECK_VALUE"]))
                    {
                        param.Tables[0].Rows.Add
                        (
                            Util.UserInfo.CorporationCode, Util.UserInfo.BusinessCode, parameters[i]["PLANT_GB"], parameters[i]["LINE_GB"], parameters[i]["VEND_CD"], Util.UserInfo.UserID, Util.UserInfo.LanguageShort
                        );
        ...
        public bool Validation(DataRow parameter, Library.ActionType actionType, int actionRow = -1)
        {
            ...
            {
                if (String.IsNullOrEmpty(parameter["PLANT_GB"].ToString()) || parameter["PLANT_GB"].ToString().Trim().Equals(""))
                    this.MsgCodeAlert_ShowFormat("SRMALC00-0001", actionRow, Util.GetHeaderColumnName(Grid01, "PLANT_GB"));

                else if (String.IsNullOrEmpty(parameter["LINE_GB"].ToString()) || parameter["LINE_GB"].ToString().Trim().Equals(""))
                    this.MsgCodeAlert_ShowFormat("SRMALC00-0001", actionRow, Util.GetHeaderColumnName(Grid01, "LINE_GB"));

                else if (String.IsNullOrEmpty(parameter["VEND_CD"].ToString()) || parameter["VEND_CD"].ToString().Trim().Equals(""))
                    this.MsgCodeAlert_ShowFormat("SRMALC00-0002", actionRow, Util.GetHeaderColumnName(Grid01, "VEND_CD"));
            ...
            {
                if (String.IsNullOrEmpty(parameter["PLANT_GB"].ToString()) || parameter["PLANT_GB"].ToString().Trim().Equals(""))
                    this.MsgCodeAlert_ShowFormat("SRMALC00-0003", actionRow, Util.GetHeaderColumnName(Grid01, "PLANT_GB"));

                else if (String.IsNullOrEmpty(parameter["LINE_GB"].ToString()) || parameter["LINE_GB"].ToString().Trim().Equals(""))
                    this.MsgCodeAlert_ShowFormat("SRMALC00-0003", actionRow, Util.GetHeaderColumnName(Grid01, "LINE_GB"));

                else if (String.IsNullOrEmpty(parameter["VEND_CD"].ToString()) || parameter["VEND_CD"].ToString().Trim().Equals(""))
                    this.MsgCodeAlert_ShowFormat("SRMALC00-0003", actionRow, Util.GetHeaderColumnName(Grid01, "VEND_CD"));
            ...
            else
            {
                if (this.cdx01_VENDCD.IsEmpty)
                    this.MsgCodeAlert_ShowFormat("SRMALC00-0004", "CDX01_VENDCD", lbl01_VEND.Text);
