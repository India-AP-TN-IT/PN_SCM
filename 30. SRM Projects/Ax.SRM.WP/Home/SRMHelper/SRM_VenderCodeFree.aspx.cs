...
namespace Ax.SRM.WP.Home.SRMHelper
{
    ...
    /// - 작 성 일 : 2014-07-28<br />
    /// </summary>
    public partial class SRM_VenderCodeFree : BasePage
    {
        /// <summary>
        /// SRM_VenderCode 생성자
        /// </summary>
        public SRM_VenderCodeFree()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    this.txt01_VENDCD_FREE.Text = HttpUtility.ParseQueryString(sQuery).Get("CODE");
                    this.txt01_VENDNM_FREE.Text = HttpUtility.ParseQueryString(sQuery).Get("TEXT");

                    
                    this.OPTION.SetValue(HttpUtility.ParseQueryString(sQuery).Get("OPTION"));
        ...
        public void GridPanel_RowDblClick(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                X.Js.Call("fn_sendParentWindow", this.txt01_ID.Text, parameters[0]["VENDCD"], parameters[0]["VENDNM"], parameters[0]["VENDCD"], JSON.Serialize(parameters[0]));
            }
            catch (Exception ex)
            {
                this.ErrorMessageAlert(this, ex);  // Error message server logging and Display message on UI Screen
        ...
        public void GridDataBind()
        {
            try
            {
                ...
                param.Add("VENDCD", this.txt01_VENDCD_FREE.Value);
                param.Add("VENDNM", this.txt01_VENDNM_FREE.Value);
                param.Add("LANG_SET", Util.UserInfo.LanguageShort);

                
                if (HttpUtility.ParseQueryString(Request.Url.Query).Get("OPTION") != null)
                {
                    string option = this.OPTION.Value.ToString();
        ...
        public void Reset()
        {
            this.txt01_VENDCD_FREE.Value = "";
            this.txt01_VENDNM_FREE.Value = "";
