...
namespace Ax.SRM.WP.Home.SRMHelper
{
    ...
    /// - 작 성 일 : 2014-10-16<br />
    /// </summary>
    public partial class SRM_PopCustPlantcdGrid : BasePage
    {
        /// <summary>
        /// SRM_PopCustPlantcdGrid 생성자
        /// </summary>
        public SRM_PopCustPlantcdGrid()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    this.txt01_PLANTCD.Text = HttpUtility.ParseQueryString(sQuery).Get("CODE");
                    this.txt01_PLANTNM.Text = HttpUtility.ParseQueryString(sQuery).Get("TEXT");
                    this.txt01_VENDORCD.Text = HttpUtility.ParseQueryString(sQuery).Get("VENDORCD");
        ...
        public void GridPanel_RowDblClick(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                X.Js.Call("fn_sendParentWindow", this.txt01_ID.Text, parameters[0]["PLANTCD"], parameters[0]["PLANTNM"], parameters[0]["PLANTCD"], JSON.Serialize(parameters[0]));
            }
            catch (Exception ex)
            {
                this.ErrorMessageAlert(this, ex);  // Error message server logging and Display message on UI Screen
        ...
        public void GridDataBind()
        {
            try
            {
                ...
                param.Add("VENDORCD", this.txt01_VENDORCD.Text);
                param.Add("PLANTCD", this.txt01_PLANTCD.Value);
                param.Add("PLANTNM", this.txt01_PLANTNM.Value);
                param.Add("LANG_SET", Util.UserInfo.LanguageShort);

                result = EPClientHelper.ExecuteDataSet("APG_EPHELPWINDOW.INQUERY_CUST_PLANTCD", param);
        ...
        public void Reset()
        {
            this.txt01_PLANTCD.Value = "";
            this.txt01_PLANTNM.Value = "";
        ...
        public void Accept(string json)
        {
            try
            {
                ...
                {
                    X.Js.Call("fn_sendParentWindow", this.txt01_ID.Text, parameter[0]["PLANTCD"], parameter[0]["PLANTNM"], parameter[0]["PLANTCD"], JSON.Serialize(parameter[0]));
                }
                else
                {
