...
namespace Ax.SRM.WP.Home.SRMHelper
{
    ...
    /// - 작 성 일 : 2014-09-02<br />
    /// </summary>
    public partial class SRM_PopCustItemcdGrid : BasePage
    {
        /// <summary>
        /// PopCustItemcdGrid 생성자
        /// </summary>
        public SRM_PopCustItemcdGrid()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    this.txt01_CUST_ITEMCD.Text = HttpUtility.ParseQueryString(sQuery).Get("CODE");
                    this.txt01_CUST_ITEMNM.Text = HttpUtility.ParseQueryString(sQuery).Get("TEXT");
                    this.txt01_VENDCD.Text = HttpUtility.ParseQueryString(sQuery).Get("VENDCD");
        ...
        public void GridPanel_RowDblClick(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                X.Js.Call("fn_sendParentWindow", this.txt01_ID.Text, parameters[0]["CUST_ITEMCD"], parameters[0]["CUST_ITEMNM"], parameters[0]["CUST_ITEMCD"], JSON.Serialize(parameters[0]));
            }
            catch (Exception ex)
            {
                this.ErrorMessageAlert(this, ex);  // Error message server logging and Display message on UI Screen
        ...
        public void GridDataBind()
        {
            try
            {
                ...
                param.Add("VENDCD", this.txt01_VENDCD.Text);
                param.Add("CUST_ITEMCD", this.txt01_CUST_ITEMCD.Value);
                param.Add("CUST_ITEMNM", this.txt01_CUST_ITEMNM.Value);
                param.Add("LANG_SET", Util.UserInfo.LanguageShort);

                result = EPClientHelper.ExecuteDataSet("APG_EPHELPWINDOW.INQUERY_CUST_ITEMCD", param);
        ...
        public void Reset()
        {
            this.txt01_CUST_ITEMCD.Value = "";
            this.txt01_CUST_ITEMNM.Value = "";
        ...
        public void Accept(string json)
        {
            try
            {
                ...
                {
                    X.Js.Call("fn_sendParentWindow", this.txt01_ID.Text, parameter[0]["CUST_ITEMCD"], parameter[0]["CUST_ITEMNM"], parameter[0]["CUST_ITEMCD"], JSON.Serialize(parameter[0]));
                }
                else
                {
