...
namespace Ax.SRM.WP.Home.SRMHelper
{
    ...
    /// - 작 성 일 : 2014-09-02<br />
    /// </summary>
    public partial class SRM_PopLineSeq : BasePage
    {
        /// <summary>
        /// PopCustItemcdGrid 생성자
        /// </summary>
        public SRM_PopLineSeq()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    this.txt01_LINECD.Text = HttpUtility.ParseQueryString(sQuery).Get("CODE");
                    this.txt01_LINENM.Text = HttpUtility.ParseQueryString(sQuery).Get("TEXT");
                    ...
                    this.txt01_INPUT_DATE.Text = HttpUtility.ParseQueryString(sQuery).Get("INPUT_DATE");
        ...
        public void GridPanel_RowDblClick(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                X.Js.Call("fn_sendParentWindow", this.txt01_ID.Text, parameters[0]["LINECD"], parameters[0]["LINENM"], parameters[0]["LINECD"], JSON.Serialize(parameters[0]));
            }
            catch (Exception ex)
            {
                this.ErrorMessageAlert(this, ex);  // Error message server logging and Display message on UI Screen
        ...
        public void GridDataBind()
        {
            try
            {
                ...
                param.Add("INPUT_DATE", this.txt01_INPUT_DATE.Text);
                param.Add("LINECD", this.txt01_LINECD.Value);
                param.Add("LINENM", this.txt01_LINENM.Value);
                param.Add("LANG_SET", Util.UserInfo.LanguageShort);

                result = EPClientHelper.ExecuteDataSet("APG_EPHELPWINDOW.INQUERY_LINE_SEQ", param);
