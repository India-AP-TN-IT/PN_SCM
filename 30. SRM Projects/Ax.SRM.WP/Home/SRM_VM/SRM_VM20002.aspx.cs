...
{
    public partial class SRM_VM20002 : BasePage
    {
        private string pakageName = "APG_SRM_VM20002";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //dt.Columns.Add("SIZE");
                //dt.Columns.Add("FILE");
                //dt.Columns.Add("SUB_VENDNM");
                //dt.Rows.Add("1", "테스트", null, null, "테스트");
                //this.Store12.DataSource = dt;
                //this.Store12.DataBind();
        ...
        public void Print()
        {
            ...
            report.ReportName = "1000/SRM_VM/SRM_VM20001_SUBKD";
            ...
            DataSet ds7 = new DataSet(); //서연이화 거래 품목정보(2차~4차 협력사 현황)
            ...
            ds7.Tables.Add(ds.Tables[5].Copy());
            ...
            //ds7.Tables[0].TableName = "DATA";
            //ds7.Tables[0].WriteXml(Server.MapPath("/Report") + "/" + report.ReportName + "07.xml", XmlWriteMode.WriteSchema);
            ...
            HERexSection xmlSectionSub7 = new HERexSection(ds7, new HEParameterSet());
            ...
            report.Sections.Add("XML7", xmlSectionSub7);
        ...
        public void Print2()
        {
            ...
            HERexReport report = new HERexReport(this.UserInfo.UserID, this.UserInfo.UserName, true);
            report.ReportName = "1000/SRM_VM/SRM_VM20002_SUBKD";

            // Main Section ( 메인리포트 파라메터셋 )
            HERexSection mainSection = new HERexSection();
            mainSection.ReportParameter.Add("PRINT_USER", this.UserInfo.UserID + "(" + this.UserInfo.UserName + ")");
            mainSection.ReportParameter.Add("MENUID", "SRM_VM20002");
            mainSection.ReportParameter.Add("ASRM2160_STD_YEAR", this.txt03_STD_YEAR.Value);
            ...
            DataSet ds4 = new DataSet(); //서연이화 거래 품목정보(2차~4차 협력사 현황)
            ds4.Tables.Add(ds.Tables[4].Copy());
        ...
        public void cdx01_PARTNO_AfterValidation(object sender, DirectEventArgs e)
        {
            Get_ASRM2180();
        }
        ...
        private void ClearInput(ControlCollection controls)
        {
            ...
            foreach (Control ctrl in controls)
            {
                ...
                if (cdxField != null)
                {
                    if (ctrl.ID.Equals("cdx01_VENDCD") || ctrl.ID.Equals("cdx01_PARTNO"))
        ...
        {
            this.YEAR_PURCAMT.Text = string.Format(Library.getLabel("SRM_VM20002", "YEAR_PURCAMT"), this.txt02_STD_YEAR.Value.ToString());
        ...
        private void Get_ASRM2160()
        {
            ...
            for (int i = source.Tables[0].Rows.Count + 1; i < 3; i++)
            {
                source.Tables[0].Rows.Add(i, "", "", "", "", "", "", null, null, "", null, null, "", null, null);
            }
        ...
        #region [ASRM2180 : 서연이화 거래 품목정보(2차~4차 협력사 현황) 저장 ]

        //서연이화 거래 품목정보(2차~4차 협력사 현황)
        private void Get_ASRM2180()
        {
            ...
            param.Add("VENDCD", this.cdx01_VENDCD.Value);
            param.Add("LANG_SET", this.UserInfo.LanguageShort);
            param.Add("PARTNO", this.cdx01_PARTNO.TypeCD);
            DataSet source = EPClientHelper.ExecuteDataSet(string.Format("{0}.{1}", pakageName, "INQUERY_ASRM2180"), param);


            //this.Store_Grid06.RemoveAll();
            this.Store_Grid06.DataSource = source.Tables[0];
            this.Store_Grid06.DataBind();
        }
        ...
        /// </summary>
        /// <param name="e"></param>
        public void Save_ASRM2180(DirectEventArgs e)
        {
            try
            {
                string json = e.ExtraParams["Values"];
                ...
                DataSet param = Util.GetDataSourceSchema
                (
                    "CORCD", "VENDCD", "DEGREE", "PARTNO", "PROCNM", "SUB_VENDNM", "ADDR", "PHONE", "CHARGER","GRADE", "F_CORP", "REMARK", "SQ_YN", "R_ID", "USER_ID","OBJECT_ID","EMAIL"
                );

                for (int i = 0; i < parameter.Length; i++)
                {        
                    
                    if (parameter[i]["DEGREE"] == null || parameter[i]["DEGREE"] == string.Empty)
                    {
                        this.MsgCodeAlert_ShowFormat("SRMMP00-0001", (i + 1), Util.GetHeaderColumnName(Grid06, "DEGREE"));
                        return;
                    }
                    else if (parameter[i]["PARTNO"] == null || parameter[i]["PARTNO"] == string.Empty)
                    {
                        this.MsgCodeAlert_ShowFormat("SRMMP00-0001", (i + 1), Util.GetHeaderColumnName(Grid06, "PARTNO"));
                        return;
                    }    
                    else if (parameter[i]["PROCNM"] == null || parameter[i]["PROCNM"] == string.Empty)
                    {
                        this.MsgCodeAlert_ShowFormat("SRMMP00-0001", (i + 1), Util.GetHeaderColumnName(Grid06, "PROCNM"));
                        return;
                    }
                    else if (parameter[i]["SUB_VENDNM"] == null || parameter[i]["SUB_VENDNM"] == string.Empty)
                    {
                        this.MsgCodeAlert_ShowFormat("SRMMP00-0001", (i + 1), Util.GetHeaderColumnName(Grid06, "SUB_VENDNM"));
                        return;
                    }                    
                    //else if (parameter[i]["DEGREE"].ToString().Equals("1") && (parameter[i]["PARTNO"] == null || parameter[i]["PARTNO"] == string.Empty))
                    //{
                    //    this.MsgCodeAlert_ShowFormat("SRMMP00-0001","Grid06", (i + 1).ToString(), Util.GetHeaderColumnName(Grid06, "PARTNO"));
                    //    return;
                    //}                    
                    //else if (parameter[i]["PROCNM"] == null || parameter[i]["PROCNM"] == string.Empty)
                    //{
                    //    this.MsgCodeAlert_ShowFormat("SRMMP00-0001", "Grid06", (i + 1).ToString(), Util.GetHeaderColumnName(Grid06, "PROCNM"));
                    //    return;
                    //}
                    //else if (parameter[i]["SUB_VENDNM"] == null || parameter[i]["SUB_VENDNM"] == string.Empty)
                    //{
                    //    this.MsgCodeAlert_ShowFormat("SRMMP00-0001", "Grid06", (i + 1).ToString(), Util.GetHeaderColumnName(Grid06, "SUB_VENDNM"));
                    //    return;
                    //}
                    //else if (parameter[i]["CHARGER"] == null || parameter[i]["CHARGER"] == string.Empty)
                    //{
                    //    this.MsgCodeAlert_ShowFormat("SRMMP00-0001", "Grid06", (i + 1).ToString(), Util.GetHeaderColumnName(Grid06, "CHARGER"));
                    //    return;
                    //}
                    //else if (parameter[i]["F_CORP"] == null || parameter[i]["F_CORP"] == string.Empty)
                    //{
                    //    this.MsgCodeAlert_ShowFormat("SRMMP00-0001", "Grid06", (i + 1).ToString(), Util.GetHeaderColumnName(Grid06, "F_CORP"));
                    //    return;
                    //}
                    //else if (parameter[i]["S_CORP"] == null || parameter[i]["S_CORP"] == string.Empty)
                    //{
                    //    this.MsgCodeAlert_ShowFormat("SRMMP00-0001", "Grid06", (i + 1).ToString(), Util.GetHeaderColumnName(Grid06, "S_CORP"));
                    //    return;
                    //}
                    //else if (parameter[i]["SQ_YN"] == null || parameter[i]["SQ_YN"] == string.Empty)
                    //{
                    //    this.MsgCodeAlert_ShowFormat("SRMMP00-0001", "Grid06", (i + 1).ToString(), Util.GetHeaderColumnName(Grid06, "SQ_YN"));
                    //    return;
                    //}

                    param.Tables[0].Rows.Add(
                         Util.UserInfo.CorporationCode, this.cdx01_VENDCD.Value,
                         parameter[i]["DEGREE"], parameter[i]["PARTNO"], parameter[i]["PROCNM"], parameter[i]["SUB_VENDNM"], parameter[i]["ADDR"],
                         parameter[i]["PHONE"], parameter[i]["CHARGER"], parameter[i]["GRADE"], parameter[i]["F_CORP"], parameter[i]["REMARK"], 
                         parameter[i]["SQ_YN"], parameter[i]["R_ID"], Util.UserInfo.UserID,parameter[i]["OBJECT_ID"], parameter[i]["EMAIL"]
                    );
                }

                using (EPClientProxy proxy = new EPClientProxy())
                {
                    proxy.ExecuteNonQueryTx(string.Format("{0}.{1}", pakageName, "SAVE_ASRM2180"), param);
                    this.MsgCodeAlert("COM-00902");
                    this.Get_ASRM2180();
                }
        ...
        /// <summary>
        ///서연이화 거래 품목정보(2차~4차 협력사 현황) 삭제
        /// </summary>
        /// <param name="e"></param>
        public void Remove_ASRM2180(DirectEventArgs e)
        {
            try
            {
                string json = e.ExtraParams["Values"];
                ...
                {
                    if (parameter[i]["CHECK_VALUE"].ToString().ToUpper().Equals("TRUE"))
                    {
                    ...
                    //HEParameterSet paramcheck = new HEParameterSet();
                    //paramcheck.Add("CORCD", this.UserInfo.CorporationCode);
                    //paramcheck.Add("VENDCD", this.cdx01_VENDCD.Value);
                    //paramcheck.Add("DEGREE", parameter[i]["DEGREE"]);
                    //DataTable checkDt = EPClientHelper.ExecuteDataSet(string.Format("{0}.{1}", pakageName, "INQUERY_SRM2180_CHECK"), paramcheck).Tables[0];
                    //if (checkDt.Rows[0]["CHECK_VAL"].ToString().Equals("F")) { this.MsgCodeAlert_ShowFormat("SRMMM-0023", new string[] { checkDt.Rows[0]["DEGREE"].ToString() }); return; } 
                }
                ...
                using (EPClientProxy proxy = new EPClientProxy())
                {
                    proxy.ExecuteNonQueryTx(string.Format("{0}.{1}", pakageName, "DELETE_ASRM2180"), param);
                    this.MsgCodeAlert("COM-00903");
                    this.Get_ASRM2180();
                }
        ...
        public void Search()
        {
            try
            {
                ...
                #region [서연이화 거래 품목정보(2차~4차 협력사 현황)]
                this.Get_ASRM2180();
                this.Get_ASRM2180_FILE();
                #endregion
        ...
        private DataSet getDataSetReport()
        {
            ...
            return EPClientHelper.ExecuteDataSet(string.Format("{0}.{1}", pakageName, "INQUERY_REPORT"), param, new string[] { "OUT_CURSOR", "OUT_CURSOR_ASRM2130", "OUT_CURSOR_ASRM2140", "OUT_CURSOR_ASRM2150", "OUT_CURSOR_ASRM2160", "OUT_CURSOR_ASRM2180" });
        }
        ...
        private DataSet getDataSetReport2()
        {
            ...
            return EPClientHelper.ExecuteDataSet(string.Format("{0}.{1}", pakageName, "INQUERY_REPORT2"), param, new string[] { "OUT_CURSOR", "OUT_CURSOR_ASRM2090", "OUT_CURSOR_ASRM2150", "OUT_CURSOR_ASRM2160", "OUT_CURSOR_ASRM2180" });
        }
        #endregion
        ...
        public void Save_ASRM2010(ArrayList ds, ArrayList proc)
        {
            ...
            {
                DataSet param = Util.GetDataSourceSchema(
                                 "CORCD", "VENDCD", "VAATZCD", "VEND_TIER", "UP_VEND", "SUBCONTR_YN",
                ...
                ds.Add(param);
                proc.Add("APG_SRM_VM20002.SAVE_ASRM2010");
            }
            catch (Exception ex)
            {
                this.ErrorMessageAlert(this, ex);  // Error message server logging and Display message on UI Screen
        ...
        public void Save_ASRM2020(ArrayList ds, ArrayList proc)
        {
            try
            {
                ...
                ds.Add(param);
                proc.Add("APG_SRM_VM20002.SAVE_ASRM2020");
            }
            catch (Exception ex)
            {
                this.ErrorMessageAlert(this, ex);  // Error message server logging and Display message on UI Screen
        ...
        public void Save_ASRM2030(ArrayList ds, ArrayList proc)
        {
            try
            {
                ...
                ds.Add(param);
                proc.Add("APG_SRM_VM20002.SAVE_ASRM2030");
            }
            catch (Exception ex)
            {
                this.ErrorMessageAlert(this, ex);  // Error message server logging and Display message on UI Screen
        ...
        public void Save_ASRM2040(ArrayList ds, ArrayList proc)
        {
            try
            {
                ...
                ds.Add(param);
                proc.Add("APG_SRM_VM20002.SAVE_ASRM2040");
            }
            catch (Exception ex)
            {
                this.ErrorMessageAlert(this, ex);  // Error message server logging and Display message on UI Screen
        ...
        public void Save_ASRM2060(ArrayList ds, ArrayList proc)
        {
            try
            {
                ...
                ds.Add(param);
                proc.Add("APG_SRM_VM20002.SAVE_ASRM2060");
            }
            catch (Exception ex)
            {
                this.ErrorMessageAlert(this, ex);  // Error message server logging and Display message on UI Screen
        ...
        public void Save_ASRM2070(ArrayList ds, ArrayList proc)
        {
            try
            {
                ...
                ds.Add(param);
                proc.Add("APG_SRM_VM20002.SAVE_ASRM2070");
            }
            catch (Exception ex)
            {
                this.ErrorMessageAlert(this, ex);  // Error message server logging and Display message on UI Screen
        ...
        public void Save_ASRM2080(ArrayList ds, ArrayList proc)
        {
            try
            {
                ...
                ds.Add(param);
                proc.Add("APG_SRM_VM20002.SAVE_ASRM2080");
            }
            catch (Exception ex)
            {
                this.ErrorMessageAlert(this, ex);  // Error message server logging and Display message on UI Screen
        ...
        public void Save_ASRM2110(ArrayList ds, ArrayList proc)
        {
            try
            {
                DataSet param = Util.GetDataSourceSchema(
                                 "CORCD", "VENDCD",
                                 "BIZ_CODE1", "SQ_CATEGORY1", "SQ_GRADE1", "CERT_DATE1", "VALID_DATE1", "MAIN_CHARGE1",
                                 "BIZ_CODE2", "SQ_CATEGORY2", "SQ_GRADE2", "CERT_DATE2", "VALID_DATE2", "MAIN_CHARGE2",
                                 "BIZ_CODE3", "SQ_CATEGORY3", "SQ_GRADE3", "CERT_DATE3", "VALID_DATE3", "MAIN_CHARGE3",
                                 "USER_ID");

                param.Tables[0].Rows.Add
                        (
                            this.UserInfo.CorporationCode, this.cdx01_VENDCD.Value,
                            this.txt01_BIZCD1.Value, this.cdx01_SQ_CATEGORY1.Value, this.cbo01_SQ_GRADE1.Value, this.getDateString(this.df01_CERT_DATE1), this.getDateString(this.df01_VALID_DATE1), this.txt01_MAIN_CHARGE1.Value,
                            this.txt01_BIZCD2.Value, this.cdx01_SQ_CATEGORY2.Value, this.cbo01_SQ_GRADE2.Value, this.getDateString(this.df01_CERT_DATE2), this.getDateString(this.df01_VALID_DATE2), this.txt01_MAIN_CHARGE2.Value,
                            this.txt01_BIZCD3.Value, this.cdx01_SQ_CATEGORY3.Value, this.cbo01_SQ_GRADE3.Value, this.getDateString(this.df01_CERT_DATE3), this.getDateString(this.df01_VALID_DATE3), this.txt01_MAIN_CHARGE3.Value,
                            this.UserInfo.UserID
                        );

                ds.Add(param);
                proc.Add("APG_SRM_VM20002.SAVE_ASRM2110");
            }
            catch (Exception ex)
            {
                this.ErrorMessageAlert(this, ex);  // Error message server logging and Display message on UI Screen
        ...
        public void Save_ASRM2120(ArrayList ds, ArrayList proc)
        {
            try
            {
                ...
                ds.Add(param);
                proc.Add("APG_SRM_VM20002.SAVE_ASRM2120");
            }
            catch (Exception ex)
            {
                this.ErrorMessageAlert(this, ex);  // Error message server logging and Display message on UI Screen
        ...
        private void Excel_Import()
        {
            try
            {
                if (string.IsNullOrEmpty(this.cdx01_VENDCD.Value))
                {
                    this.MsgCodeAlert_ShowFormat("COM-00901", "cdx01_VEND", lbl01_VEND.Text);
                    return;
                }
                ...
                (
                    "CORCD", "VENDCD", "DEGREE", "PARTNO", "PARTNM", "PROCNM", "SUB_VENDNM", "ADDR", "CHARGER", "GRADE", "PHONE", "EMAIL", "F_CORP","REMARK", "SQ_YN", "R_ID", "USER_ID"
                );

                for (int i = 2; i < saveParam.Tables[0].Columns.Count; i++)
                {
                    dtTemp.Columns.Add(saveParam.Tables[0].Columns[i].ColumnName);
                }
                for (int i = 0; i < dtUpload.Rows.Count; i++)
                {
                    if (dtUpload.Rows[i]["PARTNO"] != null && !string.IsNullOrEmpty(dtUpload.Rows[i]["PARTNO"].ToString()))
                    {
                ...
                for (int i = 0; i < dtTemp.Rows.Count; i++)
                {
                    ...
                    (
                        ...
                        , this.cdx01_VENDCD.Value
                        , dtTemp.Rows[i]["DEGREE"]
                        , dtTemp.Rows[i]["PARTNO"]
                        , dtTemp.Rows[i]["PARTNM"]
                        , dtTemp.Rows[i]["PROCNM"]
                        , dtTemp.Rows[i]["SUB_VENDNM"]
                        , dtTemp.Rows[i]["ADDR"]
                        , dtTemp.Rows[i]["CHARGER"]
                        , dtTemp.Rows[i]["GRADE"]                        
                        , dtTemp.Rows[i]["PHONE"]
                        , dtTemp.Rows[i]["EMAIL"]
                        , dtTemp.Rows[i]["F_CORP"]
                        //, dtTemp.Rows[i]["S_CORP"]
                        , dtTemp.Rows[i]["REMARK"]
                        , dtTemp.Rows[i]["SQ_YN"]
                        , dtTemp.Rows[i]["R_ID"]                        
                        , this.UserInfo.UserID                         
                    );

                    if (dtTemp.Rows[i]["DEGREE"] == null || dtTemp.Rows[i]["DEGREE"].ToString() == string.Empty)
                    {
                        this.MsgCodeAlert_ShowFormat("SRMMP00-0001", (i + 1), Util.GetHeaderColumnName(Grid06, "DEGREE"));
                        return;
                    }
                    else if (dtTemp.Rows[i]["PARTNO"] == null || dtTemp.Rows[i]["PARTNO"].ToString() == string.Empty)
                    {
                        this.MsgCodeAlert_ShowFormat("SRMMP00-0001", (i + 1), Util.GetHeaderColumnName(Grid06, "PARTNO"));
                        return;
                    }
                    else if (dtTemp.Rows[i]["PROCNM"] == null || dtTemp.Rows[i]["PROCNM"].ToString() == string.Empty)
                    {
                        this.MsgCodeAlert_ShowFormat("SRMMP00-0001", (i + 1), Util.GetHeaderColumnName(Grid06, "PROCNM"));
                        return;
                    }
                    else if (dtTemp.Rows[i]["SUB_VENDNM"] == null || dtTemp.Rows[i]["SUB_VENDNM"].ToString() == string.Empty)
                    {
                        this.MsgCodeAlert_ShowFormat("SRMMP00-0001", (i + 1), Util.GetHeaderColumnName(Grid06, "SUB_VENDNM"));
                        return;
                    }
                    //else if (dtTemp.Rows[i]["DEGREE"].ToString().Equals("1") && (dtTemp.Rows[i]["PARTNO"] == null || dtTemp.Rows[i]["PARTNO"].ToString() == string.Empty))
                    //{
                    //    this.MsgCodeAlert_ShowFormat("SRMMP00-0001", "Grid06", (i + 1).ToString(), Util.GetHeaderColumnName(Grid06, "PARTNO"));
                    //    return;
                    //}                    
                ...
                if (saveParam.Tables[0].Rows.Count > 0)
                {
                    using (EPClientProxy proxy = new EPClientProxy())
                    {
                        proxy.ExecuteNonQueryTx(string.Format("{0}.{1}", pakageName, "SAVE_ASRM2180_EXCEL"), saveParam);
                        this.MsgCodeAlert("COM-00902");
                        Get_ASRM2180();
                    }                   
                }
        ...
        private void Excel_Export()
        {
            try
            {
                ...
                param.Add("LANG_SET", this.UserInfo.LanguageShort);
                param.Add("PARTNO", this.cdx01_PARTNO.TypeCD);
                DataSet result = EPClientHelper.ExecuteDataSet(string.Format("{0}.{1}", pakageName, "INQUERY_ASRM2180_EXCEL"), param);
                ...
                {
                    ExcelHelper.ExportExcel(this.Page, result.Tables[0], Grid06);
                }
            }
            catch (Exception ex)
            {
        ...
        #region [ 이벤트 ]
        [DirectMethod]
        public void MultiUpload1_FileUpload(object sender, FileUploadEventArgs e)
        {
            ...
            DataSet param = Util.GetDataSourceSchema(
                 "CORCD","VENDCD", "SUB_VENDNM",
                 "FILE_NAME", "BLOB$FILE_DATA",
                 "USER_ID");
            ...
            using (EPClientProxy proxy = new EPClientProxy())
            {
                proxy.ExecuteNonQueryTx(string.Format("{0}.{1}", pakageName, "FILE_ASRM2180"), param);
                this.Get_ASRM2180_FILE();
                this.MsgCodeAlert("EP20010-002");
            }
        }

        //서연이화 거래 품목정보(2차~4차 협력사 현황) 파일
        private void Get_ASRM2180_FILE()
        {

            HEParameterSet param = new HEParameterSet();
            param.Add("CORCD", this.UserInfo.CorporationCode);
            param.Add("VENDCD", this.cdx01_VENDCD.Value);
            DataSet source = EPClientHelper.ExecuteDataSet(string.Format("{0}.{1}", pakageName, "INQUERY_ASRM2180_FILE"), param);

            this.Store12.DataSource = source.Tables[0];
            this.Store12.DataBind();
        }

        public void Remove_ASRM2180_FILE(object sender, DirectEventArgs e)
        {
            try
            {
                //업체코드 필수입력
                ...
                using (EPClientProxy proxy = new EPClientProxy())
                {
                    proxy.ExecuteNonQueryTx(string.Format("{0}.{1}", pakageName, "DELETE_ASRM2180_FILE"), param);
                    this.MsgCodeAlert("COM-00903");
                    this.Get_ASRM2180_FILE();
                }
        ...
        [DirectMethod]
        public void SeeFile(string r_id)
        {
            HEParameterSet param = new HEParameterSet();
            param.Add("R_ID", r_id);
            DataSet source = EPClientHelper.ExecuteDataSet(string.Format("{0}.{1}", pakageName, "INQUERY_ASRM2180_FILE_DOWNLOAD"), param);

            if (source.Tables[0].Rows.Count > 0)
            {
                if (source.Tables[0].Rows[0]["FILE_DATA"] != System.DBNull.Value)
                {
                    byte[] ADD_FILE = (byte[])source.Tables[0].Rows[0]["FILE_DATA"];
                    string ADD_FILENM = source.Tables[0].Rows[0]["FILE_NAME"].ToString();

                    string tmp_dir = @"C:\Temp";
                    if (!Directory.Exists(tmp_dir))
                        Directory.CreateDirectory(tmp_dir);

                    string[] file_info = ADD_FILENM.Split('.');
                    string tmp_file = String.Format(@"{0}\{1}.{2}", tmp_dir, Guid.NewGuid().ToString(), file_info[file_info.Length - 1]);
                    if (File.Exists(tmp_file))
                        File.Delete(tmp_file);

                    int ArraySize = ADD_FILE.GetUpperBound(0);
                    FileStream stream = new FileStream(tmp_file, FileMode.OpenOrCreate, FileAccess.Write);
                    stream.Write(ADD_FILE, 0, ArraySize + 1);
                    stream.Close();
                    Thread.Sleep(1000);
                    Process.Start(tmp_file);
