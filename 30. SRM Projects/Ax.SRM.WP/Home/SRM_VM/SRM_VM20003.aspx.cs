...
/* 
 * 프로그램명 : 업체 기본현황 작성
 * 설      명 : Home > 협력업체관리 > 업체정보관리 > 3,4차사 기본정보등록
 * 최초작성자 : 배명희
 * 최초작성일 : 2015-01-30
 * 
 * 1) 서연이화 로그인시
 ...
 * 2) 주관장사 로그인시
 *    업체코드 : 입력가능
 *               SRM2010에 데이터 없거나, SRM2010의 UP_VEND정보가 NULL이거나, 
 *               [SRM2010.UP_VEND = 로그인한 VENDCD]인 MM3110.SQ_VENDCD만 조회 가능.
 *    주관장사코드 : 입력 불가. 자신의 업체코드로 고정
 * 
 * 
 * 3) 3,4차사 로그인시 
 *    업체코드 : 입력 불가. 자신의 USER INFO에 해당하는 MM3110.SQ_VENDCD 고정
 *    주관장사코드 : 입력 불가. 자신의 USER INFO에 해당하는 VENDCD로 디폴트 고정
 *                   But, 한 업체당 여러군데의 주관장사가 있을수 있음.
 *                   데이터 조회시 SRM2010에 데이터 존재하는 경우 SRM2010.UP_VEND를 박아주고(등록된 내용으로 표시한다는 의미),
 *                                 SRM2010에 데이터 미존재하는 경우 자신의 USER INFO에 해당하는 VENDCD로 디폴트 고정한다.
 * 수정  내용 :
 *  
 *				날짜			작성자		이슈
 *				---------------------------------------------------------------------------------------------
...
{
    public partial class SRM_VM20003 : BasePage
    {
        private string pakageName = "APG_SRM_VM20003";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!X.IsAjaxRequest)
                {
                    ...
                    //if (this.UserInfo.UserDivision.Equals("T16"))
                    //{
                    //    this.Search();                  
                    //}
        ...
        public void MakeButton(string id, string image, string alt, Ext.Net.Panel container, bool isUpload = false)
        {
            ...
            if (ibtn.ID.Equals(ButtonID.Search))
            {
                ...
                ibtn.DirectEvents.Click.EventMask.CustomTarget = "UIContainer";
        ...
        public void fileDelete(string fileID)
        {
            try
            {
                ...
                param.Add("VENDCD", this.cdx01_SQ_VEND.Value);
        ...
        public void Print2()
        {
            ...
            HERexSection mainSection = new HERexSection();
            mainSection.ReportParameter.Add("PRINT_USER", this.UserInfo.UserID + "(" + this.UserInfo.UserName + ")");
            mainSection.ReportParameter.Add("MENUID", "SRM_VM20003");
            mainSection.ReportParameter.Add("ASRM2160_STD_YEAR", this.txt03_STD_YEAR.Value);
        ...
        {
            //this.Reset(false);
            //this.cdx01_SQ_VEND.SetValue(string.Empty);
        }
        ...
        public void CodeBox_BeforeDirectButtonClick(object sender, DirectEventArgs e)
        {
            ...
            if (this.UserInfo.UserDivision.Equals("T12"))
                cdx.UserParamSet.Add("VENDCD", string.Empty);
            else if (this.UserInfo.UserDivision.Equals("T15"))
                cdx.UserParamSet.Add("VENDCD", this.UserInfo.VenderCD);
            if (this.UserInfo.UserDivision.Equals("T16"))
                cdx.UserParamSet.Add("VENDCD", string.Empty);
        ...
        public void CodeBox_CustomRemoteValidation(object sender, EP.UI.EPCodeBox_ValidationResult rsltSet)
        {
            ...
            param.Add("BIZCD", "");
            if (this.UserInfo.UserDivision.Equals("T12"))
                param.Add("VENDCD", string.Empty);
            else if (this.UserInfo.UserDivision.Equals("T15"))
                param.Add("VENDCD", this.UserInfo.VenderCD);
            else
                param.Add("VENDCD", string.Empty);
            param.Add("SQ_VENDCD", cdx.TypeCD);
            param.Add("LANG_SET", Util.UserInfo.LanguageShort);
            param.Add("USER_ID", Util.UserInfo.UserID);

            DataSet ds = EPClientHelper.ExecuteDataSet("APG_SRM_VM20003.INQUERY_VEND_VALIDATION", param);

            rsltSet.resultDataSet = ds;                                             //  결과 데이터셋
            rsltSet.resultValidation = ds.Tables[0].Rows.Count == 0 ? false : true; // 동일한 데이터가 있으면 true 없으면 false
            rsltSet.returnValueFieldName = "SQ_VENDCD";
            rsltSet.returnTextFieldName = "SQ_VENDNM";
        }
        ...
        public void CodeBox_AfterValidation(object sender, DirectEventArgs e)
        {
            ...
            this.Reset(false);
            if (!this.cdx01_SQ_VEND.IsEmpty && this.cdx01_SQ_VEND.isValid)
                this.Search();
            else
            {
                if (this.UserInfo.UserDivision.Equals("T12"))
        ...
        public void Reset(bool isAllReset)
        {
            if (isAllReset)
            {
                //로그인한 사람이 2차협업체 사람인 경우(자신의 SQ_VENDCD를 업체코드에 고정)
                if (this.UserInfo.UserDivision.Equals("T16"))
                {
                    string sq_vendcd = this.GetUserSQVendcd();

                    cdx01_SQ_VEND.InnerTextField_TypeCD.RemoteValidation.Clear();

                    // 처음 값 처리
                    cdx01_SQ_VEND.isEditable = true;
                    cdx01_SQ_VEND.SetValue(sq_vendcd);
                    cdx01_SQ_VEND.isEditable = false;

                    cdx01_SQ_VEND.InnerTextField_TypeCD.ReadOnly = true;
                    cdx01_SQ_VEND.InnerTextField_TypeNM.ReadOnly = true;
                    cdx01_SQ_VEND.InnerImageButton_Helper.Enabled = false;
                    cdx01_SQ_VEND.VisiableHelper = false;
                }
                else
                {
                    //3,4차 업체코드 초기화
                ...
                //로그인 사용자가 협력업체 및 2차협력업체인 경우
                //주관장사코드에 자신의 VENDCD를 고정
                if (this.UserInfo.UserDivision.Equals("T15") || this.UserInfo.UserDivision.Equals("T16"))
                {
                    cdx01_VENDCD.InnerTextField_TypeCD.RemoteValidation.Clear();

                    // 처음 값 처리
                    cdx01_VENDCD.isEditable = true;
                    cdx01_VENDCD.SetValue(this.UserInfo.VenderCD);
                    cdx01_VENDCD.isEditable = false;

                    cdx01_VENDCD.InnerTextField_TypeCD.ReadOnly = true;
                    cdx01_VENDCD.InnerTextField_TypeNM.ReadOnly = true;
                    cdx01_VENDCD.InnerImageButton_Helper.Enabled = false;
                    cdx01_VENDCD.VisiableHelper = false;
                }
                else
                {
                    //업체코드는 서연이화 사용자인 경우에만 초기화한다.
        ...
        private void ClearInput(ControlCollection controls)
        {
            ...
            foreach (Control ctrl in controls)
            {
                ...
                if (cdxField != null)
                {
                    ...
                    else if (ctrl.ID.Equals("cdx01_SQ_VEND"))
                        continue;                                   //3,4치 업체코드는 초기화하지 않음   
                ...
                if (numField != null)
                {
                    ...
                    {
                        numField.SetValue(DateTime.Now.Year - 1);    //기준년도는 전년도로 세팅        
        ...
        /// <summary>
        /// 로그인한 사용자가 2차 협력업체 사람인 경우 MM3120에서 자신의 SQ_VENDCD 조회
        /// </summary>
        /// <returns></returns>
        private string GetUserSQVendcd()
        {
            HEParameterSet param = new HEParameterSet();
            param.Add("CORCD", this.UserInfo.CorporationCode);
            ...
            DataSet ds = EPClientHelper.ExecuteDataSet("APG_SRM_RM30001.INQUERY_USER_SQ_VENDCD", param);
            if (ds.Tables[0].Rows.Count > 0)
        ...
        public void Save_AVM2090()
        {
            try
            {
                ...
                param.Tables[0].Rows.Add
                        (
                            this.UserInfo.CorporationCode, this.cdx01_SQ_VEND.Value,
        ...
        {
            this.YEAR_PURCAMT.Text = string.Format(Library.getLabel("SRM_VM20003", "YEAR_PURCAMT"), this.txt02_STD_YEAR.Value.ToString());
        ...
        public void Save_ASRM2100(DirectEventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < parameter.Length; i++)
                {
                    ...
                    param.Tables[0].Rows.Add(
                         Util.UserInfo.CorporationCode, this.cdx01_SQ_VEND.Value, this.txt02_STD_YEAR.Value,
        ...
        public void Save_ASRM2130(DirectEventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < parameter.Length; i++)
                {
                    ...
                    param.Tables[0].Rows.Add(
                         Util.UserInfo.CorporationCode, this.cdx01_SQ_VEND.Value,
        ...
        public void Search()
        {
            try
            {
                ...
                if (source.Tables[0].Rows.Count > 0)
                {
                    ...
                    string UP_VEND = dr["UP_VEND"].ToString();

                    if (this.UserInfo.UserDivision.Equals("T16"))
                    {
                        //로그인한 사람이 2차협업체 사람인 경우
                        if (!UP_VEND.Equals(string.Empty))
                        {

                            this.cdx01_VENDCD.InnerTextField_TypeCD.Text = UP_VEND;
                            this.cdx01_VENDCD.InnerTextField_TypeNM.Text = dr["UP_VENDNM"].ToString();
                        }

                    }
                    else if (this.UserInfo.UserDivision.Equals("T15"))
                    {
                        //로그인 사용자가 협력업체 사람인 경우
                        if (!UP_VEND.Equals(string.Empty) && !this.cdx01_VENDCD.Value.Equals(UP_VEND))
                        {
                            this.MsgCodeAlert("SRMVM-0010"); //다른 주관장사에 이미 등록되었습니다.
                            return;
                        }
                    }
                    else
                    {
                        //로그인 사용자가 서연이화 사람인 경우
                        this.cdx01_VENDCD.SetValue(dr["UP_VEND"]);
                    }
                    ...
                    this.txt01_VENDNM22.SetValue(dr["VENDNM"]);
        ...
        public void Save(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                this.Save_VM2020(arrDataSet, arrProcedure); //대표자정보
                this.Save_VM2030(arrDataSet, arrProcedure); //임직원 및 노조정보
                this.Save_VM2040(arrDataSet, arrProcedure); //품질인증정보
                this.Save_VM2050(arrDataSet, arrProcedure); //품목정보
                this.Save_VM2060(arrDataSet, arrProcedure); //주주현황
                this.Save_VM2070(arrDataSet, arrProcedure); //관계사현황
                this.Save_VM2080(arrDataSet, arrProcedure); //해외진출
                //this.Save_VM2090(arrDataSet, arrProcedure); //재무제표
                //this.Save_VM2100(arrDataSet, arrProcedure); //매입현황
                this.Save_VM2110(arrDataSet, arrProcedure); //SQ인증정보
                this.Save_VM2120(arrDataSet, arrProcedure); //기타정보
        ...
        /// <param name="ds"></param>
        /// <param name="proc"></param>
        public void Save_VM2020(ArrayList ds, ArrayList proc)
        {
            try
            {
                DataSet param = Util.GetDataSourceSchema(
        ...
        /// <param name="ds"></param>
        /// <param name="proc"></param>
        public void Save_VM2030(ArrayList ds, ArrayList proc)
        {
            try
            {
                DataSet param = Util.GetDataSourceSchema(
        ...
        /// <param name="ds"></param>
        /// <param name="proc"></param>
        public void Save_VM2040(ArrayList ds, ArrayList proc)
        {
            try
            {
                DataSet param = Util.GetDataSourceSchema(
        ...
        /// <param name="ds"></param>
        /// <param name="proc"></param>
        public void Save_VM2050(ArrayList ds, ArrayList proc)
        {
            try
            {
                DataSet param = Util.GetDataSourceSchema(
        ...
        /// <param name="ds"></param>
        /// <param name="proc"></param>
        public void Save_VM2060(ArrayList ds, ArrayList proc)
        {
            try
            {
                DataSet param = Util.GetDataSourceSchema(
        ...
        /// <param name="ds"></param>
        /// <param name="proc"></param>
        public void Save_VM2070(ArrayList ds, ArrayList proc)
        {
            try
            {
                DataSet param = Util.GetDataSourceSchema(
        ...
        /// <param name="ds"></param>
        /// <param name="proc"></param>
        public void Save_VM2080(ArrayList ds, ArrayList proc)
        {
            try
            {
                DataSet param = Util.GetDataSourceSchema(
        ...
        /// <param name="ds"></param>
        /// <param name="proc"></param>
        public void Save_VM2110(ArrayList ds, ArrayList proc)
        {
            try
            {
                DataSet param = Util.GetDataSourceSchema(
        ...
        /// <param name="ds"></param>
        /// <param name="proc"></param>
        public void Save_VM2120(ArrayList ds, ArrayList proc)
        {
            try
            {
        ...
        {
            //3,4 업체코드 필수입력
            if (this.cdx01_SQ_VEND.IsEmpty)
            {
                this.MsgCodeAlert_ShowFormat("EP20S01-003", "cdx01_SQ_VEND", this.lbl02_VEND.Text); //{0}를(을) 입력해주세요.
                return false;
            }
            ...
            {
                this.MsgCodeAlert_ShowFormat("EP20S01-003", "cdx01_VENDCD", this.lbl02_UP_VENDNM.Text); //{0}를(을) 입력해주세요.
                return false;
            }
        ...
        public bool IsQueryValidation()
        {
            ...
            if (this.cdx01_SQ_VEND.IsEmpty)
            {
                this.MsgCodeAlert_ShowFormat("EP20S01-003", "cdx01_SQ_VEND", lbl02_VEND.Text);
                return false;
            }
